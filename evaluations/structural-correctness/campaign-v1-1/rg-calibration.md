...
The historical review design selected every one of the 54 candidate IDs before labeling in `rg-semantic-labeling-runs/pr-208.batch-001.manifest.json`. The final proposed labels name the source-evidence proposer `deepseek-v4-flash` and the campaign-local adjudicator `deepseek-v4-pro`; their immutable run records, execution contracts, and authority boundaries are copied into the generated provenance. This task makes no new model call and does not relabel, repair, or extend #309. The generated analysis retains the declared label, its original semantic witnesses, the candidate-anchor source witnesses, reviewing method/identity, and the observed retrieval fact as separate fields.
